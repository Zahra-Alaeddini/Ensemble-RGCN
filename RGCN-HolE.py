...
def evaluate(links):
    ...
    with torch.no_grad():
        ...
        rel_types = edge_type_tensor[src_nodes]

        src_embeds = node_embeddings[src_nodes]
        dst_embeds = node_embeddings[dst_nodes]
        scores = decoder(src_embeds, dst_embeds, rel_types)
...
decoder = HolEDecoder(out_dim=out_dim, num_rels=num_rels).to(device)
...
def save_results(results, filename='HolE_fold_results.csv'):

...
class SimplEDecoder(nn.Module):
    def __init__(self, out_dim):
        ...
        self.head_embeddings = nn.Parameter(torch.randn(out_dim, device=device))
        self.tail_embeddings = nn.Parameter(torch.randn(out_dim, device=device))
        self.rel_forward_embeddings = nn.Parameter(torch.randn(out_dim, device=device))
        self.rel_inverse_embeddings = nn.Parameter(torch.randn(out_dim, device=device))

    def forward(self, src_embeds, dst_embeds):
        forward_score = torch.sum(src_embeds * self.rel_forward_embeddings * dst_embeds, dim=1)
        inverse_score = torch.sum(dst_embeds * self.rel_inverse_embeddings * src_embeds, dim=1)
        scores = torch.sigmoid(0.5 * (forward_score + inverse_score))
        return scores
...
decoder = SimplEDecoder(out_dim=out_dim).to(device)
...
def save_results(results, filename='SimplE_fold_results.csv'):
